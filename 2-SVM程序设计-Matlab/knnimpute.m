...
% $Revision: 1.1.12.8 $   $Date: 2009/07/06 20:28:24 $
...
if nargin > 2
    if rem(nargin,2) == 1
        error('Bioinfo:knnimpute:IncorrectNumberOfArguments',...
    ...
    for j=1:2:nargin-2
        ...
        if isempty(karg)
            error('Bioinfo:knnimpute:UnknownParameterName',...
                'Unknown parameter name: %s.',pname);
        elseif length(karg)>1
            error('Bioinfo:knnimpute:AmbiguousParameterName',...
...
if strncmpi(metric,'mahalanobis',max(length(metric),2)) && rank(cov(dataNoNans'))<size(dataNoNans,1)
    warning('Bioinfo:knnimpute:DependentColumns',...
        'When using the Mahalanobis distance, there should be more independent columns than rows without NaNs. Try a different distance metric.')
end
